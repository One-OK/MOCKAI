...
interface ExportStepProps {
  mockupUrl: { withRulers: string, withoutRulers: string };
  onRestart: () => void;
  lang: Language;
}
...
export const ExportStep: React.FC<ExportStepProps> = ({ mockupUrl, onRestart, lang }) => {
  ...
  const downloadAll = () => {
    // Download production mockup (with rulers)
    const link1 = document.createElement('a');
    link1.download = `mockai-production.png`;
    link1.href = mockupUrl.withRulers;
    link1.click();

    // Small delay to ensure both downloads trigger
    setTimeout(() => {
      // Download visual mockup (without rulers)
      const link2 = document.createElement('a');
      link2.download = `mockai-visual.png`;
      link2.href = mockupUrl.withoutRulers;
      link2.click();
    }, 500);
  };
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8 animate-in fade-in zoom-in duration-500">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        <div className="lg:col-span-2 bg-white p-4 rounded-3xl shadow-2xl border border-slate-200 overflow-hidden group">
          <div className="relative rounded-2xl overflow-hidden bg-slate-100">
            <img 
              src={mockupUrl.withRulers} 
              alt="Final Mockup" 
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="text-left">
                      ...
                      <div className="text-[10px] text-slate-300 font-normal">包含生产图和效果图</div>
                    </div>
                  </div>
